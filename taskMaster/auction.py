...
async def _sealed_bid(
    received_values, task, task_id, price_selector, machines, auction
):
    """
    ...
    if len(sorted_values) > 1:
        reward_value = (
            sorted_values[1]["bid"]
            if price_selector == "SPSB"
            else sorted_values[0]["bid"] / 2
        )
        non_winner_sockets = [
            machine[1]
            ...
            if machine[0] == sorted_values[0].get("id"):
                ...
                if len(non_winner_sockets) > 0:
                    websockets.broadcast(
                        non_winner_sockets,
                        json.dumps({"winner": False, "task_id": task_id}),
                    )
                    break
    ...
    await winner[1].send(
        json.dumps(
            {"winner": True, "reward": reward_value, "task": task, "task_id": task_id}
        )
    )
...
async def auction_result(result, auction, offloading_parameters):
    ...
    if offloading_parameters.get("deadline_seconds") <= time_taken:
        late_tasks.append(
            (
                offloading_parameters.get("task_id"),
                time_taken - offloading_parameters.get("deadline_seconds"),
            )
        )
    results.append(
        {
            "time": time.time() - auction.get("start_machine_timer"),
            "result": result,
            "shape_numbers": offloading_parameters.get("shape_numbers"),
            "deadlines": offloading_parameters.get("deadlines"),
            "machines": auction.get("machines"),
        }
    )
