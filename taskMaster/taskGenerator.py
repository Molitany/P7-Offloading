...
def generate_tasks(
    ...
    for i in range(0, amount):
        ...
        deadline_difference = (
            max_deadline - deadline_seconds
            if max_deadline - deadline_seconds > 0
            else max_deadline
        )
        offloading_parameters["deadline_seconds"] = deadline_seconds
        offloading_parameters["max_reward"] = deadline_difference * _average(
            shape_numbers
        )
        offloading_parameters["fines"] = (
            offloading_parameters.get("max_reward") * 2
            if offloading_parameters.get("fines")
            else 0
        )
