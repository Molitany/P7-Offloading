...
async def establish_server():
    ...
    async with websockets.serve(
        new_connection, host, port, max_size=None, ping_timeout=60
    ) as websocket:
...
async def handle_server():
    ...
    while True:
        ...
        if (
            ...
            and completed_tasks == to_be_completed
        ):
            log_task()
        ...
        if len(client_inputs) > 0:
            ...
            for _ in range(0, batches):
                ...
                task_queue.extend(
                    generate_tasks(
                        amount=frequency
                        if frequency != -1 and frequency < amount
                        else amount,
...
def log_task():
    ...
    logger.log_colored_message(
        logger.colors.GREEN,
        f'Size of tasks: {result.get("shape_numbers")}',
    )
    logger.log_colored_message(
        logger.colors.GREEN,
    ...
    if result.get("deadlines"):
        logger.log_colored_message(
            logger.colors.GREEN, f"late tasks: {len(late_tasks)/completed_tasks*100}%"
        )
        logger.log_colored_message(
            logger.colors.GREEN,
        ...
        if task_delays:  # If there is no delays then max complains
            logger.log_colored_message(
                logger.colors.GREEN, f"Maximum of task delays: {max(task_delays)}"
            )
    else:
        logger.log_colored_message(logger.colors.GREEN, f"Maximum of task delays: {0}")
    machines_connected = set([len(result.get("machines")) for result in results])
    for amount in machines_connected:
        logger.log_colored_message(logger.colors.GREEN, f"Machines connected: {amount}")
    shutil.copyfile(
        "log.txt",
        f'logs/finished_log_{datetime.today().isoformat(sep=" ", timespec="seconds").replace(" ", "_").replace(":", "-")}.txt',
    )
    ...
    while True:
        try:
            ...
            if isinstance(received, dict):
                ...
                if auction:
                    if received.get("bid"):  # Bid
                        ...
                        if len(auction.get("machines")) == len(
                            auction.get("bid_results")
                        ):
                            await handle_bid(auction)
                    elif received.get("result"):  # Matrix result
                        await auction_result(
                            received, auction, auction.get("offloading_parameters")
                        )
                        completed_tasks += 1
            ...
            for auction in auctions.values():
                if False in [m in machines for m in auction.get("machines")] or len(
                    auction.get("bid_results")
                ) != len(auction.get("machines")):
