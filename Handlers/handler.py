...
async def establish_client():
    ...
    while True:
        try:
            async with connect(
                f"ws://{host}:{port}", max_size=None, ping_timeout=60
            ) as websocket:
                while True:
...
async def auction_action(websocket, recieved):
    ...
    logger.log_colored_message(
        logger.colors.BLUEHIGH,
        f'finished receiving auction {{id:{id} task:{offloading_parameters.get("task_id")}}}',
    )
    if offloading_parameters["offloading_type"] == "Auction":
        if (
            offloading_parameters["auction_type"] == "Second Price Sealed Bid"
            or offloading_parameters["auction_type"] == "SPSB"
            or offloading_parameters["auction_type"] == "FPSB"
            or offloading_parameters["auction_type"] == "First Price Sealed Bid"
        ):
...
async def winner_action(websocket, auction_result: dict):
    ...
    await websocket.send(
        json.dumps({"result": result, "task_id": auction_result.get("task_id")})
    )
    logger.log_colored_message(
        logger.colors.GREENHIGH,
        f'finished sending result {{task:{auction_result.get("task_id")}}}',
    )
...
async def bid_truthfully(offloading_parameters, websocket, id):
    ...
    estimated_cost_of_task = (
        task_difficulty_duration.get(max_shape_num, max_shape_num * 0.001)
        * IDLE_POWER_CONSUMPTION
    )
    bid_value = 0
    if internal_value < 0:
        bid_value = estimated_cost_of_task + abs(
            internal_value
        )  # add the idle time to the bid
